...
DETECT_CLASS = (Detect, Detect_LSCD, Detect_LQE,
                Detect_LSCD_LQE ,)
V10_DETECT_CLASS = (v10Detect,)
SEGMENT_CLASS = (Segment, Segment_LSCD, Segment_LQE, Segment_LSCD_LQE)
POSE_CLASS = (Pose, Pose_LSCD, Pose_LQE, Pose_LSCD_LQE)
OBB_CLASS = (OBB, OBB_LSCD, OBB_LQE, OBB_LSCD_LQE)
A2C2F_CLASS = (A2C2f,)
C2PSA_CLASS = (C2PSA,)
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        try:
            ...
            # if t == 'LightUpsample':
            #     # in_ch  = ch[f] if args[0] is None else args[0]
            #     # out_ch = args[1]            # 按 YAML 填好的值
            #     # scale  = args[2]
            #     # args   = [in_ch, out_ch, scale]
            #     m = LightUpsample
            # else:
            # # 原来的逻辑
        ...
        if m in ((
            ...
            C3x, RepC3, PSA, SCDown, C2fCIB, nn.Conv2d, CSP_SPDP, 
        ) + A2C2F_CLASS + C2PSA_CLASS):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in ((
                BottleneckCSP, C1, C2, C2f, C3k2, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fPSA, C2fCIB,CSP_SPDP
            )  + A2C2F_CLASS + C2PSA_CLASS):
            ...
            if m in SEGMENT_CLASS:
                ...
                if m in (Segment_LSCD, Segment_LSCD_LQE):
                    args[3] = make_divisible(min(args[3], max_channels) * width, 8)
            if m in (Detect_LSCD, Detect_LSCD_LQE):
                args[1] = make_divisible(min(args[1], max_channels) * width, 8)
            if m in (Pose_LSCD, Pose_LSCD_LQE, 
                     OBB_LSCD, OBB_LSCD_LQE):
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
